%% ------------ ODE DYNAMICS FUNCTION WITH FIXED E_FREE CALCULATION ------------
% This version properly solves for E_free self-consistently within each ODE evaluation
% Eliminates the conservation violation issue present in the original version

function dxdt = ode_dynamics_multipleE_FIXED(X, P)
global N PAS Ef_ss
...
if P.FirstRun
    ...
    % Verify conservation (optional, for debugging)
    if isfield(P, 'verify_conservation') && P.verify_conservation
        REvalbindEAfterPas = RE_val_bind_E(Ef_ss);
        E_used_verify = sum(R(1:N)'.* RE_val_bind_E(Ef_ss)) + ...
                        sum(REH' .* REvalbindEAfterPas(PAS:N));
        E_balance = P.E_total - E_used_verify - Ef_ss;
        
        if abs(E_balance) > 1e-3
            warning('Conservation check failed: E_balance = %.4e', E_balance);
        end
    end
end

%% Standard ODE dynamics (unchanged)
